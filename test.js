...
test('it scans a pullable source', (t) => {
  ...
  const scanned = scan((prev, x) => prev + x, 0)(source);
  const sink = makeSink();
  scanned(0, sink);
...
test('it scans a pullable source without a seed', (t) => {
  ...
  const source = makeSource();
  const scanned = scan((prev, x) => prev + x)(source);
  const sink = makeSink();
  scanned(0, sink);
...
test('it scans an async finite listenable source', (t) => {
  t.plan(16);
  const upwardsExpected = [
    [0, 'function']
  ];
  ...
  const source = makeSource();
  const scanned = scan((acc, x) => acc + x, 0)(source);
  scanned(0, sink);
...
test('it scans a listenable source without a seed', (t) => {
  t.plan(16);
  const upwardsExpected = [
    [0, 'function']
  ];
  ...
  const source = makeSource();
  const scanned = scan((acc, x) => acc + x)(source);
  scanned(0, sink);
